...
/*
	Checks to see if every single tile spot has been claimed by a player (game is a draw)
*/
...
/*
	Initialize the game struct on the heap
*/
Game* InitGame() {
...
/*
	Initialize a player struct on the heap
*/
...
/*
	Fully deallocates a game's dynamic memory
*/
...
/*
	Send information about a player to the player
*/
int SendPlayerInfo(Player* player) {
	...
	RemoveChars(buffer, "\n\t\0");
	return SendData(player->Client, buffer);
}

/*
	Requests the player send it's move to the server
*/
...
/*
	Tells a player that the last move it made was not valid, with an error code which explains why
*/
...
/*
	Sends the game data to the connected players
*/
void BroadcastTable(Game* game) {
	...
	DeallocJsonParser(parser);
	RemoveChars(buffer, "\n\t\0");
	SendData(game->PlayerOne->Client, buffer);
	SendData(game->PlayerTwo->Client, buffer);
}

/*
	Sends the result of the game (play won, draw, etc.) to the connected players
*/
